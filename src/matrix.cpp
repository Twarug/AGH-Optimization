...
void matrix::set_col(const matrix &c, int mv) {
  ...
  if (n != c.n)
    throw string("void matrix::set_col(const matrix&,int):\nliczba wierszy "
                 "macierzy musi byc rowna dlugosci wektora");
  if (c.m != 1)
    throw string("void matrix::set_col(const matrix&,int):\nwstawiana kolumna "
                 "musi miec postac wektora pionowego");
  for (int i = 0; i < n; ++i)
    M[i][mv] = c(i);
}
...
void matrix::set_row(const matrix &c, int nv) {
  ...
  if (c.n != 1)
    throw string("void matrix::set_row(const matrix&,int):\nwstawiany wiersz "
                 "musi miec postac wektora poziomego");
  for (int i = 0; i < m; ++i)
    M[nv][i] = c(0, i);
}
...
bool operator<(const matrix &A, const matrix &B) {
  ...
  if (nA[0] != 1 || nA[1] != 1 || nB[0] != 1 || nB[1] != 1)
    throw string("bool operator<(const matrix&, const matrix&):\noperator "
                 "relacji jest zdefiniwany tylko dla macierzy 1x1");
  return A() < B();
}
...
double m2d(const matrix &A) {
  int *nA = get_size(A);
  if (nA[0] != 1 || nA[1] != 1)
    throw string("double m2d(const matrix&):\nzamiana macierzy na liczbe "
                 "mozliwa jest tylko dla skalarow");
  return A(0, 0);
}
...
double norm(const matrix &A) {
  int *nA = get_size(A);
  if (nA[1] != 1)
    throw string("double norm(const matrix&):\nnorma jest zdefiniowana tylko "
                 "dla wektorow pionowych");
  double N = 0;
  for (int i = 0; i < nA[0]; ++i)
    N += pow(A(i), 2);
  return sqrt(N);
