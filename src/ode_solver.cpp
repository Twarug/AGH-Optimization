...
matrix *solve_ode(matrix (*diff)(double, matrix, matrix, matrix), double t0,
                  double dt, double tend, matrix Y0, matrix ud1, matrix ud2) {
  try {
    int N = static_cast<int>(floor((tend - t0) / dt) + 1);
    if (N < 2)
      throw string("matrix* solve_ode(...):\nprzedzial czasu nie jest "
                   "zdefiniowany poprawnie");
    int *s = get_size(Y0);
    if (s[1] != 1)
      throw string("matrix* solve_ode(...):\nwarunek poczatkowy musi byc "
                   "wektorem pionowym");
    int n = s[0];
    delete[] s;
    ...
    for (int i = 1; i < N; ++i) {
      ...
      for (int j = 0; j < n; ++j)
        S[1](j, i) =
            S[1](j, i - 1) + (k1(j) + 2 * k2(j) + 2 * k3(j) + k4(j)) / 6;
    }
    S[1] = trans(S[1]);
    return S;
