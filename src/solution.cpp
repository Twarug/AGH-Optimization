...
matrix solution::fit_fun(matrix (*ff)(matrix, matrix, matrix), matrix ud1,
                         matrix ud2) {
  try {
    ++f_calls;
    y = ff(x, ud1, ud2);
...
matrix solution::grad(matrix (*gf)(matrix, matrix, matrix), matrix ud1,
                      matrix ud2) {
  try {
    ++g_calls;
    g = gf(x, ud1, ud2);
...
matrix solution::hess(matrix (*Hf)(matrix, matrix, matrix), matrix ud1,
                      matrix ud2) {
  try {
    ++H_calls;
    H = Hf(x, ud1, ud2);
